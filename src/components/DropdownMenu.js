    ...
    return (
      <div className="childrens">
        {parent.childrens.map((child, id) => (
          ...
          >
            {child.name}
            {parseInt(whichHover) === parseInt(child.id) ? (
              <>
                <Childrens
                  parentID={whichHover}
                  famillyArray={parent.childrens}
                />{" "}
              </>
            ) : null}
          </div>
...
export default function DropdownMenu({ famillyArray }) {
  ...
  return (
    <div className="menu paper" onMouseLeave={() => setWhichHover(null)}>
      <div className="menuMainCard">
        {famillyArray.map((category, id) => (
          <div
            className={
              parseInt(whichHover) === parseInt(category.id)
                ? "mainCategory paper focused"
                : "mainCategory paper"
            }
            id={category.id}
            key={id}
